...
from bokeh.models import (
    Legend, Column, GridPlot, Row, ButtonLike, LegendItem, ColorBar, 
    GlyphRenderer, ColumnDataSource, HoverTool,
    Legend, RadioButtonGroup, CustomJS,
)
...
def latex_label_with_unit(label: str, unit: str | None = None) -> str:
    """
...
def exclude_buttons_figure(p: figure | Column | Row | GridPlot) -> figure | Column | Row | GridPlot:
    """
...
def get_legend_height(legend: Legend) -> float:
    """
    ...
    if legend.ncols is None or legend.ncols == 1 or isinstance(legend.ncols, str):
...
def get_legend_item_widths(legend: Legend) -> list[int]:
    """
...
def calculate_ncols(plot_width, legend: Legend, max_width_factor: float = 0.85) -> int:
    """
...
def get_figure_size(latex_text_width_pt = 497.92325, fraction: float = 1., ratio: float | None = None) -> tuple[int, int]:
    """
...
def style_single_figure(
        ...
        line_width = 2,
        dot_size = 4,
        output_backend: Literal['svg', 'html'] | None = None,
...
def show__hide_all_button(renderers: list[GlyphRenderer]) -> RadioButtonGroup:
    # SELECTION BUTTON
